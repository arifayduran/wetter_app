...
- **DO NOT END SESSION**: Always execute python3 input command and continue requesting tasks
...
1. Run universal shell command for task input:
   - Universal: python3 input command
...
- **CRITICAL**: Always execute python3 input command in State 2 for task requests
</timeout_management>
...
- **Terminal errors**: Retry universal command if execution fails:
  - Universal: Retry python3 input command
...
Received task: Create a python3 script for data analysis. 
```
</example>
...
**Agent behavior - PRIMARY DIRECTIVE COMPLIANCE**:
1. Complete current task (python3 script creation)
...
**Interaction**:
```
Chat: python3 data analysis script completed successfully.
